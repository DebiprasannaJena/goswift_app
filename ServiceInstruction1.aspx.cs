...
public partial class ServiceInstruction1 : System.Web.UI.Page
{
                ...
                if (Session["SvcMasterData"] != null)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            //// URL Formation                          
                            string strUrl = "ServiceInstruction1.aspx?ReqMode=M&FormId=" + strServiceId + "&ServiceType=" + intServiceType + "&ProposalNo=" + strProposalNo + "&Amount=" + decAmount;
    ...
    protected void BtnProceed_Click(object sender, EventArgs e)
    {
        try
        {
            if (Int_ServiceType == 0) //For Internal Service
            {
                ...
                {
                    Response.Redirect("FormView1.aspx?FormId=" + str_FormId + "&ProposalNo=" + str_ProposalNo + "&ReqMode=" + str_RequestMode);
                }
                else
                {
                    DataTable dt = (DataTable)Session["SvcMasterData"];
                    ...
                    {
                        string urlchk = "FormView1.aspx?FormId=" + str_FormId + "&ProposalNo=" + str_ProposalNo + "&ReqMode=" + str_RequestMode;
                        dr["vchUpdateUrl"] = urlchk;
                        Response.Redirect("FormView1.aspx?FormId=" + str_FormId + "&ProposalNo=" + str_ProposalNo + "&ReqMode=" + str_RequestMode);
                    }
                }
