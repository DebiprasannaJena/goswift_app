...
public partial class incentives_Grantprioritysectorsstatus : SessionCheck
{
    ...
    {
        string retval = "";
        retval = IncentiveManager.IsPriorityApp(intInvestorId, 1);
        eligbilemsg = retval;
        return retval;
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (ChkPriorityApp(Convert.ToInt16(Session["InvestorId"])) != "1")
        {
            ...
            ///*------------------------------------------------------------------------------------------------------------*/
            //// The industry must have to satisfy any 1 of the below conditions for applying Provisional Priority Certificate.
            //// 1. The industry must have a derived sector.
            //// 2. The industry belongs from backward districts.
            //// 3. The Ancillary/DownStream must be "Yes".
            ///*------------------------------------------------------------------------------------------------------------*/
            //string message = @"<strong>You are not eligible for applying provisional priority certificate.</strong>";
    ...
    protected void btnEdit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ///------------------------------------------------------------------------------------------------
            Response.Redirect("FormPreview_Grantprioritysectorsstatus.aspx?InctUniqueNo=" + Convert.ToString(mstyp));
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "OnClick", "<script> jAlert('<strong>Application Drafted Successfully !</strong>', 'GO-SWIFT'); </script>", false);
            }
        }
        catch (Exception ex) { Util.LogError(ex, "Incentive"); }
        finally { }
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    objAvailDetails.IncentiveTypeID = Convert.ToInt32(dr["intIncentiveType"].ToString());
                    objAvailDetails.QuantumValue = Convert.ToDecimal(dr["decQuantum"]);
                    objAvailDetails.AvailedEarlierFrom = Convert.ToDateTime(dr["dtmFrom"]);
                    objAvailDetails.AvailedEarlierTo = Convert.ToDateTime(dr["dtmto"]);
                    objAvailDetails.ApplicabilityId = Convert.ToInt32(dr["intIPRApplicability"].ToString());
    ...
    protected void PrioritySectorDetails()
    {
        try
        {
            ...
            objIncentive.PrioritySector.strNote = HiddenField3.Value;
