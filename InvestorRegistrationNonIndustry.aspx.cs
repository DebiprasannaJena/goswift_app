...
public partial class InvestorRegistrationNonIndustry : System.Web.UI.Page
{
    #region Variables
        ...
        try
        {
            ...
            if (sqlReader.HasRows)
            {
                while (sqlReader.Read())
                {
                    ...
                    NONINDUSTRIAL_PAGE = Convert.ToInt32(sqlReader["INT_NONINDUSTRIAL_PAGE"].ToString());
                }

                if (NONINDUSTRIAL_PAGE == 0)
                {
        ...
        try
        {
            ...
            if (arrResult[0] == "1" && arrResult[1] != "")
            {
                ...
                if (dtcontent.Rows.Count > 0)
                {
                    ...
                    /*----------------------------------------------------------------*/
                     objComm.UpdateMailSMSStaus("OTP", strMobileNo, InvToEmail[0], strSubject, "0", "0", 0, "0", strSMSContent, strSMSContent, smsStatus, mailStatus);
                }
            }
            return result;
        }
