...
function CTreeNode(treeView, parentNode , text, url, target){
	...
	this.getContent=function(){
		function itemSquare(node){
				...
				var w=parseInt(node.treeView.fmt.Fw);var h=node.treeView.fmt.Fh;
		...
		if(this.treeView.fmt.showB) s += this.hasChildren() ? buttonSquare(this) : blankSquare(this, 16);
		if(this.treeView.fmt.showF) s += itemSquare(this);
		var n = this.treeView.name;
		if(this.url == "")	
			s += this.hasChildren()? '<td nowrap=\"1\">'+this.text+'</td></tr></table>' : '<td nowrap=\"1\">'+this.text+'</td></tr></table>';
		else 
			s += '<td nowrap=\"1\">'+this.text+'</td></tr></table>';
        return s;
	}
