...
public partial class Portal_Dashboard_MinisterDashboard : SessionCheck
{
        ...
        if (Session["desId"].ToString() == "102")
        {
            ...
            if (!IsPostBack)
            {
             ...
             //   InsertCSRMaster(ddlFinacialYear);
               // ViewServiceStatus();
               // ViewServiceMaster();
                //DashboradCommon.YEARBIND(ddlspmgyear);
                //InsertSPMGStatus();
             //   BindDept();
                //ViewSPMGData();               
    ...
    private void ViewSPMGMasterData()/// Method for  bind SPMG  on master tracker
    {
        try
        {
            ...
            List<SWPDashboard> objServiceStatus = commonMethodobj.ViewSPMGMasterData(Convert.ToInt32(Session["Userid"]), Convert.ToInt32(Finyear.ToString().Split('-')[0]));
            if (objServiceStatus.Count > 0)
            {
    ...
    protected void LinkButton1_Click(object sender, EventArgs e)
    {
       ...
       // BindDistrict();
       // //FillFinYr();
       // ddlspmgyear.SelectedValue = DateTime.Now.Year.ToString();
       // FillFinYrPortlet(ddlAppaYear);
       // FillFinYrPortlet(ddlPealYear);
       // FillFinYrPortlet(ddlCSRYear);
       // FillFinYrPortlet(ddlYearEmployement);
    ...
    private void InsertViewDistrictlevelSPMGData(string recived, string resolved, string pending, string pendinglast) // STATE PROJECT MONITORING GROUP
    {
        ...
        try
        {
            ...
            //    con.Close();
            //}
            DataTable PnlDt = commonMethodobj.InsertViewDistrictlevelSPMGData(Convert.ToInt32(Session["Userid"].ToString()), Convert.ToInt32(ddlspmgyear.SelectedValue), recived, resolved, pending, pendinglast);
            if (PnlDt.Rows.Count > 0)
            {
