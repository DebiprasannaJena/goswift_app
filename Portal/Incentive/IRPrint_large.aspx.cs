...
using System.IO;
public partial class Portal_Incentive_IRPrint_large : System.Web.UI.Page
{
    ...
    //        string getextension = Path.GetExtension(fuSupportingDocuments.FileName).ToLower();
    //        if ((getextension == ".pdf") || (Path.GetExtension(fuSupportingDocuments.FileName) == ".png") || (getextension == ".jpg") || (getextension == ".jpeg"))
    //        {
    ...
    //        if ((Path.GetExtension(fuSignature.FileName) == ".png") || (extension == ".jpg") || (extension == ".jpeg"))
    //        {
    ...
    protected void btnConfirm_Click(object sender, EventArgs e)
    {
        ...
        if (intRetValue == 2 || intRetValue == 1)
        {
            if (string.Equals(strCommandArg, "d", StringComparison.OrdinalIgnoreCase))
            {
                ...
                //ScriptManager.RegisterStartupScript(btnConfirm, this.GetType(), "Myalert", string.Format("alert('{0}');window.location.href='IRFormLarge.aspx?ID={1}&linkm={2}&linkn={3}&btn={4}&tab={5}&ranNum={6}&pSize={7}&pIndex={8}&AppNo={9}&status={10}&dist={11}&uname={12}';", Messages.ShowMessage("2"), Request.QueryString["ID"], Request.QueryString["linkm"], Request.QueryString["linkn"], Request.QueryString["btn"], Request.QueryString["tab"], Session["RandomNo"], Request.QueryString["pSize"], Request.QueryString["pIndex"], Request.QueryString["AppNo"], Request.QueryString["status"], Request.QueryString["dist"], Request.QueryString["uname"]), true);
            }
            else
            {
    ...
    {
        Response.Redirect(string.Format("../Incentive/ViewIncentiveApplication.aspx?ID={1}&linkm={2}&linkn={3}&btn={4}&tab={5}&ranNum={6}&pSize={7}&pIndex={8}&AppNo={9}&status={10}&dist={11}&uname={12}';", Request.QueryString["ID"], Request.QueryString["linkm"], Request.QueryString["linkn"], Request.QueryString["btn"], Request.QueryString["tab"], Session["RandomNo"], Request.QueryString["pSize"], Request.QueryString["pIndex"], Request.QueryString["AppNo"], Request.QueryString["status"], Request.QueryString["dist"], Request.QueryString["uname"]));
    }
    #endregion
    ...
    private void GetApplicationDetails(DataSet objDs)
    {
        if (objDs != null && objDs.Tables.Count > 0)
        {
            ...
            if (dtPcDetails != null && dtPcDetails.Rows.Count > 0)
            {
                ...
                //for old data added by Suman on 9th october,2017
