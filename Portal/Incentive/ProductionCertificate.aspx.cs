/*
 * File Name :- ProductionCertificate.aspx.cs
 * Class name : - Portal_Incentive_ProductionCertificate
 * [Modification History]
 * [cr no.]     [Modified On]       [Modified By]       [Description]
 *  1           18th Nov 2017       Ritika Lath         Added code to redirect page to pcprint page to create pdf
 */
...
public partial class Portal_Incentive_ProductionCertificate : SessionCheck
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            int intStatus = objBuisness.PcPrintDetails_AED(objMaster);

            //if record saved successfully
            if (intStatus == 2)
            {
                //redirect to print pc page create the pdf
                ScriptManager.RegisterStartupScript(btnSubmit, this.GetType(), "Myalert", string.Format("alert('{0}');window.location.href='PcPrint.aspx?ID={1}&linkm={2}&linkn={3}&btn={4}&tab={5}&ranNum={6}&pcupstat={7}&type=1';", Messages.ShowMessage("2"), Request.QueryString["ID"], Request.QueryString["linkm"], Request.QueryString["linkn"], Request.QueryString["btn"], Request.QueryString["tab"], Session["RandomNo"], 5), true);
            }
        }
        catch (Exception ex)
        {
