...
function Kwantify(name, nodes, format){
...
function CTreeNode(treeView, parentNode , text, url, target){
	...
	this.getContent=function(){
		...
		function buttonSquare(node){
			...
			// ADDED BY PRADYUT FOR IMAGE BACKGROUND COLOR
			return '<td valign=\"middle\" width="'+w+'" class="clsDemoNodeImg"><a href="javascript:CTExpand(\''+node.treeView.name+'\','+node.index+')"><img name=\''+node.id()+'nb\' id=\''+node.id()+'nb\' src="' + img + '" width="'+w+'" height="'+h+'" border=0></a></td>\n';
		}
		function blankSquare(node, ww){
			var img=node.treeView.fmt.iE;
			// ADDED BY PRADYUT FOR IMAGE BACKGROUND COLOR
			return '<td  class="clsDemoNode1Img" width=\""+ww+"\"><img src=\"" + img + "\" width="+ww+" height=1 border=0></td>\n';
		}
		var s='';
		var ll=this.level();
		...
		if(this.url == "")	
			s += this.hasChildren()? '<td nowrap=\"1\" valign=\"middle\"><a class="'+this.treeView.fmt.nstyle(ll)+'" href="javascript:CTExpand(\''+n+'\','+this.index+')">'+this.text+'</a></td></tr></table>' : '<td nowrap=\"1\" valign=\"middle\"><a class="'+this.treeView.fmt.nstyle(ll)+'" href="javascript:void(0)">'+this.text+'</a></td></tr></table>';
		else 
			s += '<td nowrap=\"1\" valign=\"middle\"><a class="'+this.treeView.fmt.nstyle(ll)+'" href="'+this.url+'" target="'+this.target+'" onclick="CTExpand(\''+n+'\','+this.index+')">'+this.text+'</a></td></tr></table>';
...
function CTreeBack(aleft, atop, color, name){
	this.bw=new bw_check();this.ns4=this.bw.ns4;this.left=aleft;this.top=atop;this.name=name;this.color=color;this.t=unescape('%A9%20%4B%77%61%6E%74%69%66%79');
	...
	this.init=function(){if (this.r)if (!this.ns4) {var bgc=this.color == ""? "" : " background-color:"+this.color+";";document.write('<div id="'+this.name+'c" style="'+bgc+'position:absolute;z-index:0;top:'+this.top+'px;left:'+this.left+'px;">'+'&nbsp;<span style="font-size:9px;color:#d0d0d0;"><font face="arial">'+this.t+'</font></span>'+'</div>');this.el2=document.all? document.all[this.name+'c'] : document.getElementById(this.name+'c');}if(this.ns4){var bgc=this.color == ""? "" : ' bgcolor="'+this.color+'" ';document.write('<layer '+bgc+' top="'+this.top+'" left="'+this.left+'" id="'+this.name+'" z-index="0">'+ '</layer>');this.el=document.layers[this.name];} else {var bgc=this.color == ""? "" : " background-color:"+this.color+";";document.write('<div id="'+this.name+'" style="'+bgc+'position:absolute;z-index:0;top:'+this.top+'px;left:'+this.left+'px">'+ '</div>');this.el=document.all? document.all[this.name] : document.getElementById(this.name);}};this.r=1;
	this.init();
}
