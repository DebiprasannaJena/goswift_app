...
public partial class SingleWindow_Mom : System.Web.UI.Page
{
    ...
    private void GetDetails()
    {
        ...
        try
        {
            ...
            {                
                ...
                try
                {
                    //Added by Monalisa Nayak on 26-12-2016 to bind capacity and product
                    if (ds.Tables[1].Rows.Count > 0)
                    {
                        //for (int i = ds.Tables[1].Rows.Count - 1; i > 0; i--)
                        //{
                        //    if (ds.Tables[1].Rows[i]["VCHPRODUCT"].ToString() == ds.Tables[1].Rows[i - 1]["VCHPRODUCT"].ToString())
                        //    {
                        //        ds.Tables[1].Rows[i]["VCHPRODUCT"] = "";
                        //    }
                        //}

                        rptCapacity.DataSource = ds.Tables[1];
                        rptCapacity.DataBind();
                    }
                    if (ds.Tables[8].Rows.Count > 0)
                    {
                        litPreapre.Text = Convert.ToString(ds.Tables[8].Rows[0]["VCHFULLNAME"]);
                        litChecked.Text = Convert.ToString(ds.Tables[8].Rows[1]["VCHFULLNAME"]);                        
                    }
    ...
    public void FillProjectDetails(DataSet ds)
    {
        try
        {
            if (ds.Tables[0].Rows.Count > 0)
    ...
    public void FillProposalDetails(DataSet ds)
    {
        try
        {
            ...
            {
                RptrProposal.DataSource = ds.Tables[2];
    ...
    public void FillPromoterDetails(DataSet ds)
    {
        try
        {
            if (ds.Tables[3].Rows.Count > 0)
            {
                ...
                var rows = ds.Tables[3].AsEnumerable()
                ...
                var rows1 = ds.Tables[3].AsEnumerable()
    ...
    public void FillProjInformation(DataSet ds)
    {
         try
            {
                    if (ds.Tables[4].Rows.Count > 0)
                    {
                        lblFinDesc.Text = Convert.ToString(ds.Tables[4].Rows[0]["vchFinanceDescription"].ToString());
                        if (lblFinDesc.Text.Length > 1)
                            trFin.Visible = true;
                        else
                            trFin.Visible = false;
                        lblLand.Text = Convert.ToString(ds.Tables[4].Rows[0]["vchLand"].ToString());
                        lblWater.Text = Convert.ToString(ds.Tables[4].Rows[0]["vchWater"].ToString());
                        lblPower.Text = Convert.ToString(ds.Tables[4].Rows[0]["vchPower"].ToString());

                        if ((Convert.ToString(ds.Tables[4].Rows[0]["intPowerSource"].ToString()) == "1"))
                            lblCPP.Text = "Source : CPP";
                        else if ((Convert.ToString(ds.Tables[4].Rows[0]["intPowerSource"].ToString()) == "2"))
                        {
                            lblCPP.Text = "Source : GRID";
                        }
                        else if ((Convert.ToString(ds.Tables[4].Rows[0]["intPowerSource"].ToString()) == "3"))
                        {
                            lblCPP.Text = "Source : CPP & GRID";
                        }
                        
                        lblImple.Text = Convert.ToString(ds.Tables[4].Rows[0]["vchImplementPeriod"]);
                        lblDirect.Text = Convert.ToDecimal(ds.Tables[4].Rows[0]["intEmployement"]).ToString("N2").TrimEnd('0').TrimEnd('.') + " Numbers";
                        lblContra.Text = Convert.ToDecimal(ds.Tables[4].Rows[0]["intContractual"]).ToString("N2").TrimEnd('0').TrimEnd('.') + " Numbers";
                        lblEmTotal.Text = Convert.ToDecimal(Convert.ToInt32(ds.Tables[4].Rows[0]["intEmployement"]) + Convert.ToInt32(ds.Tables[4].Rows[0]["intContractual"])).ToString("N2").TrimEnd('0').TrimEnd('.') + " Numbers";
    ...
    public void FillDecision(DataSet ds)
    {
          try
            {
                if (ds.Tables[5].Rows.Count > 0)
                {
                        DecisionText = Convert.ToString(ds.Tables[5].Rows[0]["VCHDECISION"].ToString());
                        RptrDecision.DataSource = ds.Tables[5];
                        RptrDecision.DataBind();
                }
    ...
    public void FillFinancialDetails(DataSet ds)
    {
        try
        {
            ...
            if (ds.Tables[6].Rows.Count > 0)
            {
                ...
                GrdFinanace.Columns[2].HeaderText = ds.Tables[6].Columns[3].ColumnName;
                GrdFinanace.Columns[3].HeaderText = ds.Tables[6].Columns[4].ColumnName;
                GrdFinanace.Columns[4].HeaderText = ds.Tables[6].Columns[5].ColumnName;
    ...
    public void FillSLFCMember(DataSet ds)
    {
        try
        {
            ...
            {
                RptrComments.DataSource = ds.Tables[7];
    ...
    #endregion
    /// <summary>
    /// Created by Monalisa Nayak on 26-12-2016 to bind Project Cost details
    /// </summary>
    /// <param name="ds"></param>
    public void FillProjectCostDtls(DataSet ds)
    {
        try
        {
            ...
            {
                GrdProjectCostDtls.DataSource = ds.Tables[9];
    ...
    public void FillFinancingDtls(DataSet ds)
    {
        ...
        {
            GrdFinDtls.DataSource = ds.Tables[10];
            GrdFinDtls.DataBind();
        ...
        try
        {
            if (ds.Tables[11].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[11].Rows.Count; i++)
                {
                    ...
                    string District = ds.Tables[11].Rows[i]["District"].ToString();
                    string Loc = ds.Tables[11].Rows[i]["Location"].ToString();
    ...
    public void FillMaterialSource(DataSet ds)
    {
        try
        {
            ...
            {
                GrdSource.DataSource = ds.Tables[12];
                GrdSource.DataBind();
            }
        }
        catch (Exception ex)
