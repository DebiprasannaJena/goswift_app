...
public partial class Portal_Grievance_AddGrievanceDepartment : System.Web.UI.Page
{
    ...
    private void Clearfields()
    {
        ...
        DrpDwnInvestmentLevel.ClearSelection();
        TxtGrievanceTitle.Text = "";
        DrpDwnGrivType.ClearSelection();
        DrpDwnGrivSubType.ClearSelection();
    }
    ...
    private void BindGrievanceSubType()
    {
        try
        {
            ...
            {
                StrAction = "BGST",
                intInvestorId = Convert.ToInt32(ddlInvestor.SelectedValue), //Convert.ToInt32(Session["InvestorId"].ToString()),
    ...
    private void BindUnitDetail()
    {
        try
        {
            ...
            {
                StrAction = "BUD",
                intInvestorId = Convert.ToInt32(ddlInvestor.SelectedValue)
            };
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                DrpDwnDistrict.ClearSelection();
                string strDistrictvalue =Dt.Rows[0]["INT_DISTRICT"].ToString();
                DrpDwnDistrict.SelectedValue = strDistrictvalue;
                string strcategory = Dt.Rows[0]["INT_CATEGORY"].ToString();
                DrpDwnInvestmentLevel.SelectedValue = strcategory;
    ...
    private void BindIndustry()
    {
        try
        {
            ...
            Dt = GrievanceServices.FillIndustry(objSearch);

            ddlInvestor.DataSource = Dt;
            ddlInvestor.DataTextField = "VCH_INV_NAME";
            ddlInvestor.DataValueField = "INT_INVESTOR_ID";
            ddlInvestor.DataBind();

            ListItem list = new ListItem();
            list.Text = "--Select--";
            list.Value = "0";
            ddlInvestor.Items.Insert(0, list);
        }
        catch (Exception)
        {
            throw;
    ...
    protected void ddlInvestor_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if(Convert.ToInt32(ddlInvestor.SelectedValue)>0)
        {
            BindUnitDetail();
        }
    }
    ...
    protected void BtnSearch_Click(object sender, EventArgs e)
    {
        if (Convert.ToInt32(ddlInvestor.SelectedValue) > 0)
        {
            DrpDwnGrivType.SelectedIndex = 0;
            DrpDwnGrivSubType.SelectedIndex = 0;
            TxtGrievanceTitle.Text = "";
            TxtGrievanceDetail.Text = "";
            BindUnitDetail();
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                {
                    StrAction = "SD",
                    intInvestorId = Convert.ToInt32(ddlInvestor.SelectedValue),// Convert.ToInt32(ddlInvestor.SelectedValue)
                    ...
                    IntIndustryCategory = Convert.ToInt32(DrpDwnInvestmentLevel.SelectedValue), //Convert.ToInt32(Session["IndustryType"].ToString()),  // Add by anil sahoo for Industry & non industery
                    intCreatedByDept =Convert.ToInt32(Session["UserId"].ToString())
                ...
                if (arrResult[0] == "1")
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "OnClick", "<script> jAlert('<strong>Data Saved Successfully !', '" + Messages.TitleOfProject + "', function () {location.href = '../Grievance/AddGrievanceDepartment.aspx';}); </script>", false);
                    return;
                }
                else
                {
