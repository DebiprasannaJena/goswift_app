...
public partial class Mastermodule_ViewProposal_AMS : SessionCheck
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            txtGmComments.Attributes.Add("onkeyup", string.Format("return CheckLengthKeyUp('{0}','{1}',500);", txtGmComments.ClientID, lblGmComments.ClientID));
            txtGmComments.Attributes.Add("onchange", string.Format("return checkLength('{0}','{1}',500);", txtGmComments.ClientID, lblGmComments.ClientID));
    ...
    private void BindNodalOffcr()
    {
        try
        {
            CICG_SWP_Service AddAgenda = new CICG_SWP_Service();
    ...
    public void fillGridview()
    {
        try
        {
            ...
            List<ProposalDet> objProposalList = AMSQueryServices.getProposalDetails_AMS(objProposal);
    ...
    protected void gvService_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            /*Modified by Ritika Lath for AMS Query*/
            if (gvService.DataKeys[e.Row.RowIndex].Values[6].ToString() == "Pending with GM")
            {
                ...
                {
                    //CHECK IF A QUERY HAS BEEN SENT TO INVESTOR FROM THE SLFC NETWORK
                    HiddenField hdnAmsQueryStatus= (HiddenField)e.Row.FindControl("hdnAmsQueryStatus");

                    // time elapsed for any other query or no of times query allowed has passed, no reason to go to query flow
                    if (hdnAmsQueryStatus.Value == "4" || hdnAmsQueryStatus.Value == "0")
                    {
                        if (hdnAmsQueryStatus.Value == "4")
                        {
                            e.Row.Cells[5].Text = "Pending at SLFC";
                        }
                        lblAMS.Visible = true;
                        lnkBtnAMS.Visible = false;
                    }

                   // once query has been sent, we can forward for further clarification or forward to nodal officers with intimation
                    else if (hdnAmsQueryStatus.Value == "2" || hdnAmsQueryStatus.Value == "3" || hdnAmsQueryStatus.Value == "1" || hdnAmsQueryStatus.Value == "5")
                    {
                        lnkBtnAMS.Visible = true;
    ...
    protected void btnSubmitAMS_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            CICG_SWP_Service AddAgenda = new CICG_SWP_Service();
            string retVal = "1";
            AddAgenda.AddAgenda(Proposal, ProjLoc, ProdCapcity, BrdOfDirectors, BIOfCompny, ProjCostDtls, RawMatSource, FinDetails, FinPerformance, doclist, UId);
            ...
            if (retVal == "1")
            {
                ...
                {
                    strActionCode = "add",
                    strProposalNo = hdnproposalnoAMS.Value,
                    intUserId = Convert.ToInt32(drpNodalOffcr.SelectedValue),
                    intCreatedBy = Convert.ToInt32(Session["userID"]),
                    strGmComments = txtGmComments.Text.Trim()
                };
                ...
                if (intRetValue == 1)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "OnClick", "jAlert('Agenda forwarded Successfully');", true);
                }
    ...
    protected void btnClarification_Click(object sender, EventArgs e)
    {
        AMSNodalDetails objProposalDet = new AMSNodalDetails()
        {
            strActionCode = "add",
            strProposalNo = hdnproposalnoAMS.Value,
            intUserId = Convert.ToInt32(hdnNodalOfficer.Value),
            intCreatedBy = Convert.ToInt32(Session["userID"]),
            strGmComments = txtGmComments.Text.Trim()
        };
    ...
    protected void btnForwardToNodal_Click(object sender, EventArgs e)
    {
        AMSNodalDetails objProposalDet = new AMSNodalDetails()
        {
            strActionCode = "int",
    ...
    protected void btnForwardToQuery_Click(object sender, EventArgs e)
    {
