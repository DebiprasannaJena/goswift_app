...
function blankFieldValidation(ControlName, MessageControlName, Title) {
    if ($('#' + ControlName).val().trim() == '') {
        ...
        jAlert('<strong>' + MessageControlName + ' can not be blank !</strong>', ntitle);
        $("#popup_ok").click(
      function () {
          $('#' + ControlName).focus();
...
function WhiteSpaceValidation1st(ControlName, MessageControlName, Title) {
    if ($('#' + ControlName).val().charAt(0) == ' ') {
        ...
        jAlert('<strong>White Space not allowed in first place of ' + MessageControlName + ' !</strong>', ntitle);
        return false;
    }
    else {
        return true;
...
function WhiteSpaceValidationLast(ControlName, MessageControlName, Title) {
    ...
    if (strVal.substr(strVal.length - 1) == ' ') {
        $('#' + ControlName).focus();
        jAlert('<strong>White Space not allowed in last place of ' + MessageControlName + ' !</strong>', ntitle);
        return false;
    }
    else {
        return true;
...
function SpecialCharacter1st(ControlName, MessageControlName, Title) {
    if ($('#' + ControlName).val().charAt(0) == "'") {
        jAlert('<strong> ' + MessageControlName + ' not allowed! </strong>', ntitle);
        return false;
    }
    else {
        return true;
...
function EmailValidation(Controlname, MessageControlName, Title) {
    ...
    if (strVal != '') {
        ...
        var expr = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
        if (!expr.test(strVal)) {
            jAlert('<strong>Invalid ' + MessageControlName + ' !</strong>', ntitle);
            return false;
        }
        else {
            return true;
...
function DropDownValidation(ControlName, ValueForValidate, MessageControlName, Title) {
    ...
    if ($('#' + ControlName).val() == ValueForValidate || $('#' + ControlName).val() == '') {
        jAlert('<strong>Please select ' + MessageControlName + '  !</strong>', ntitle);
        return false;
    }
    else {
        return true;
...
function checkInput(ControlName, MessageControlName, Title) {
    ...
    if (obj != '') {
        if (obj.length < 10) {
            jAlert('<strong>' + MessageControlName + ' should be atleast 10 characters long!</strong>', ntitle);
            return false;
        }
        else {
            return true;
...
function ValidBlankFile(ControlName, Title, MessageName) {
    ...
    if (file == '') {
        jAlert('<strong>' + MessageControlName + ' </strong>', ntitle);
        return false;
    }
...
function ValidFileExtention(ControlName, fileExtensions, Title) {
    ...
    if ($.inArray(extension, fileExtension) == -1) {
        jAlert('<strong> Only ' + fileExtension.join(', ') + ' file allowed  !</strong>', ntitle);
        return false;
    }
...
function ValidFileSize(ControlName, Title, SizeInMB, Units) {
    ...
    if (uploadControl.files[0].size > convertMb) {
        jAlert('<strong> File size should be less than  ' + SizeInMB + ' ' + Units + ' !</strong>', ntitle);
        $('#' + ControlName).val('');
        return false;
    }
}
...
function ValidateNumber(controlName, MessageControlName, Title) {
    var regNumber = /^\d*$/;
    if (!regNumber.test($("#" + controlName).val())) {
        jAlert('<strong> Please enter a valid ' + MessageControlName + '!</strong>', ntitle);
        $("#" + controlName).focus();
        return false;
    }
    else {
...
function ValidateFieldLength(controlName, length, MessageControlName, Title) {
    debugger;
    if ($("#" + controlName).val().length != length) {
        jAlert('<strong> ' + MessageControlName + 'should be ' + length + ' characters!</strong>', ntitle);
